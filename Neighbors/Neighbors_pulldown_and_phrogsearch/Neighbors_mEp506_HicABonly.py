...
HicAB_hits=pd.read_csv('/home/gridsan/lbrenes/hhpred/taxids/multi_gene_systems_nr/TaxIDs_nr_mEp506_HicAB_system.csv') 

# Step 2: Make a dictionary connection between the FT and Prot ID column values
FT_Prot_ID_match = dict(zip(HicAB_hits['Featuretable'], HicAB_hits['Prot_ID']))
...
for Featuretable in os.listdir(all_FT_files_dir):
    if Featuretable in FT_Prot_ID_match:
        ...
        HicAB_matches[Featuretable] = Prot_ID
...
def neighbors(directory, matches):
    ...
    for Featuretable, Prot_ID in HicAB_matches.items():
        ...
        if os.path.isfile(FT_file_path):
            ...
            if not matched_rows.empty:
                ...
                for idx in matched_indices:
                    ...
                    neighbor_rows = neighbor_rows[~neighbor_rows['non-redundant_refseq'].isin(mEp506_HicA_IDs | mEp506_HicB_IDs)]
...
def save_results(results):
    ...
    output_dir = '~/hhpred/neighbors/mEp506_HicABonly_system'
    ...
    refseq_summary_file = os.path.join(output_dir, 'mEp506_HicABonly_neighbors_prot_refseq_summary.csv')
    ...
    detailed_summary_file = os.path.join(output_dir, 'mEp506_HicABonly_neighbors_detailed_summary.csv')
