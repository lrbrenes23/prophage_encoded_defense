...
PanAT_hits=pd.read_csv('/home/gridsan/lbrenes/hhpred/taxids/multi_gene_systems_nr/TaxIDs_nr_PhiR47_1_PanAT_system.csv')

# Step 2: Make a dictionary connection between the FT and Prot ID column values
FT_Prot_ID_match = dict(zip(PanAT_hits['Featuretable'], PanAT_hits['Prot_ID']))
...
for Featuretable in os.listdir(all_FT_files_dir):
    if Featuretable in FT_Prot_ID_match:
        ...
        PanAT_matches[Featuretable] = Prot_ID
...
def neighbors(directory, matches):
    ...
    for Featuretable, Prot_ID in PanAT_matches.items():
        ...
        if os.path.isfile(FT_file_path):
            ...
            if not matched_rows.empty:
                ...
                for idx in matched_indices:
                    ...
                    neighbor_rows = neighbor_rows[~neighbor_rows['non-redundant_refseq'].isin(PhiR47_1_PanAT_Antitoxin_IDs | PhiR47_1_PanAT_Toxin_IDs)]
...
def save_results(results):
    ...
    output_dir = '~/hhpred/neighbors/PhiR47_1_PanAT_system'
    ...
    refseq_summary_file = os.path.join(output_dir, 'PhiR47_1_PanAT_system_neighbors_prot_refseq_summary.csv')
    ...
    detailed_summary_file = os.path.join(output_dir, 'PhiR47_1_PanAT_system_neighbors_detailed_summary.csv')
