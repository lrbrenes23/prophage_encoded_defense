...
mEpX1_KilAN_hits=pd.read_csv('/home/gridsan/lbrenes/hhpred/taxids/single_gene_systems_nr/TaxIDswmEpX1_KilAN_nr_dom_filtered_STRINGENT_prot.csv')

# Step 2: Make a dictionary connection between the FT and Prot ID column values
FT_Prot_ID_match = dict(zip(mEpX1_KilAN_hits['Featuretable'], mEpX1_KilAN_hits['Prot_ID']))
...
for Featuretable in os.listdir(all_FT_files_dir):
    if Featuretable in FT_Prot_ID_match:
        ...
        mEpX1_KilAN_matches[Featuretable] = Prot_ID
...
def neighbors(directory, matches):
    ...
    for Featuretable, Prot_ID in mEpX1_KilAN_matches.items():
        ...
        if os.path.isfile(FT_file_path):
            ...
            mEpX1_KilAN_system_IDs = all_prots.intersection(mEpX1_KilANs)
            ...
            if not matched_rows.empty:
                ...
                for idx in matched_indices:
                    ...
                    neighbor_rows = neighbor_rows[~neighbor_rows['non-redundant_refseq'].isin(mEpX1_KilAN_system_IDs)]
...
def save_results(results):
    ...
    output_dir = '/home/gridsan/lbrenes/hhpred/neighbors/mEpX1_KilAN_system'
    ...
    refseq_summary_file = os.path.join(output_dir, 'mEpX1_KilAN_system_neighbors_prot_refseq_summary.csv')
    ...
    detailed_summary_file = os.path.join(output_dir, 'mEpX1_KilAN_system_neighbors_detailed_summary.csv')
