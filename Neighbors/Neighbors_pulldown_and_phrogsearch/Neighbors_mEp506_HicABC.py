...
mEp506_HicCs = set(hmmerHits[hmmerHits['Domain'] == 'mEp506_HicABC_Chaperone.blts']['Query'].values.tolist())

# Step 1: Read the DataFrame
HicABC_hits=pd.read_csv('/home/gridsan/lbrenes/hhpred/taxids/multi_gene_systems_nr/TaxIDs_nr_mEp506_HicABC_system.csv') 

# Step 2: Make a dictionary connection between the FT and Prot ID column values
FT_Prot_ID_match = dict(zip(HicABC_hits['Featuretable'], HicABC_hits['Prot_ID']))
...
for Featuretable in os.listdir(all_FT_files_dir):
    if Featuretable in FT_Prot_ID_match:
        ...
        HicABC_matches[Featuretable] = Prot_ID
...
def neighbors(directory, matches):
    ...
    for Featuretable, Prot_ID in HicABC_matches.items():
        ...
        if os.path.isfile(FT_file_path):
            ...
            mEp506_HicC_IDs = all_prots.intersection(mEp506_HicCs)
            ...
            if not matched_rows.empty:
                ...
                for idx in matched_indices:
                    ...
                    neighbor_rows = neighbor_rows[~neighbor_rows['non-redundant_refseq'].isin(mEp506_HicA_IDs | mEp506_HicB_IDs | mEp506_HicC_IDs)]
...
def save_results(results):
    ...
    output_dir = '~/hhpred/neighbors/mEp506_HicABC_system'
    ...
    refseq_summary_file = os.path.join(output_dir, 'mEp506_HicABC_neighbors_prot_refseq_summary.csv')
    ...
    detailed_summary_file = os.path.join(output_dir, 'mEp506_HicABC_neighbors_detailed_summary.csv')
