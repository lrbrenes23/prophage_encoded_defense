    ...
    if mEp506_HicA_IDs and mEp506_HicB_IDs:
        ...
        for mEp506_HicA_index in mEp506_HicA_indices:
            if any(FT.iloc[mEp506_HicA_index-2:mEp506_HicA_index+3]['non-redundant_refseq'].isin(mEp506_HicB_IDs)):
                ...
                if mEp506_HicC_IDs:
                    if any(FT.iloc[mEp506_HicA_index-2:mEp506_HicA_index+3]['non-redundant_refseq'].isin(mEp506_HicC_IDs)):
                        mEp506_HicABC.append((FTloc, Acc, ID, FT.at[mEp506_HicA_index, 'non-redundant_refseq']))
                break
    
    return mEp506_HicABC
...
mEp506_HicABC_system_out = [item for sublist in results for item in sublist]

# Filter out items that do not have exactly 4 elements and print warnings
filtered_results = []
for item in mEp506_HicABC_system_out:
    if len(item) != 4:
...
mEp506_HicABC_system_df = pd.DataFrame(filtered_results, columns=['Featuretable', 'Genome', 'TaxID', 'Prot_ID'])
mEp506_HicABC_system_df.to_csv('TaxIDs_nr_mEp506_HicABC_system.csv', index=False)
